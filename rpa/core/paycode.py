...
00 Protocol fork: scan/spend keys are derived from the BIP47-style path
    m/47'/145'/0'/0'/0   (spend)
    m/47'/145'/0'/1'/0   (scan)
via the ``bip47_derive`` helper, instead of the wallet's main BIP44 receiving
chain. This isolates RPA key material from the regular spending tree.
'''
import copy
...
from . import bip47_derive


class RpaKeyError(RuntimeError):
    """Raised when the wallet cannot provide BIP47 scan/spend keys."""


def _rpa_scan_pub(wallet):
    pub = bip47_derive.scan_pubkey(wallet)
    if pub is None:
        raise RpaKeyError(
            "RPA: cannot derive scan pubkey — wallet has no BIP32 seed. "
            "00 Protocol requires m/47'/145'/0'/1'/0 derivation; "
            "legacy wallet.derive_pubkeys fallback is disabled."
        )
    return pub


def _rpa_spend_pub(wallet):
    pub = bip47_derive.spend_pubkey(wallet)
    if pub is None:
        raise RpaKeyError(
            "RPA: cannot derive spend pubkey — wallet has no BIP32 seed. "
            "00 Protocol requires m/47'/145'/0'/0'/0 derivation; "
            "legacy wallet.derive_pubkeys fallback is disabled."
        )
    return pub


def _rpa_scan_wif(wallet, password):
    wif = bip47_derive.scan_privkey_wif(wallet, password)
    if wif is None:
        raise RpaKeyError(
            "RPA: cannot export scan private key — BIP47 derivation unavailable. "
            "Legacy wallet.export_private_key_from_index fallback is disabled."
        )
    return wif


def _rpa_spend_wif(wallet, password):
    wif = bip47_derive.spend_privkey_wif(wallet, password)
    if wif is None:
        raise RpaKeyError(
            "RPA: cannot export spend private key — BIP47 derivation unavailable. "
            "Legacy wallet.export_private_key_from_index fallback is disabled."
        )
    return wif
...
from electroncash.address import Address, Base58, ScriptOutput
from electroncash.bitcoin import *  # COIN, TYPE_ADDRESS, sha256
from electroncash.i18n import _
from electroncash.transaction import Transaction, OPReturn
from electroncash.util import print_error, do_in_main_thread
...
def _calculate_paycode_shared_secret(private_key, public_key, outpoint):
    ...
    from electroncash.bitcoin import Point
    from electroncash.bitcoin import curve_secp256k1 as curve
...
def get_grind_string(wallet, prefix_size="10"):
    ...
    scanpubkey = _rpa_scan_pub(wallet)
...
def generate_paycode(wallet, prefix_size="10"):
    ...
    if networks.net.TESTNET:
        version = "05"
    scanpubkey = _rpa_scan_pub(wallet)
    spendpubkey = _rpa_spend_pub(wallet)
    expiry = "00000000"
...
def extract_private_keys_from_transaction(wallet, raw_tx, password=None):
    ...
    for input_index in range(0, min(max_inputs_as_per_rpa_spec, number_of_inputs)):
        ...
        # 00 Protocol: derived at m/47'/145'/0'/1'/0
        scanpubkey = _rpa_scan_pub(wallet)

        scan_private_key_wif_format = _rpa_scan_wif(wallet, password)
        ...
        # 00 Protocol: derived at m/47'/145'/0'/0'/0
        spendpubkey = _rpa_spend_pub(wallet)
        ...
        spendpubkey = _rpa_spend_pub(wallet)
        spend_private_key_wif_format = _rpa_spend_wif(wallet, password)

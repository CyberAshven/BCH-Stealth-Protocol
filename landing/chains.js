...
async function _xlmBalance(addr) {
  ...
  if (r.status === 404) return { balance: 0, loaded: true };
  const j = await r.json();
...
async function _xrpBalance(addr) {
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      ws.close();
      resolve({ balance: 0, loaded: false });
    }, 1e4);
...
window.chainsRefreshOne = async function(chain, addr) {
  ...
  try {
    ...
    switch (meta.apiType) {
      case "evm":
        return await _evmBalance(chain, addr);
      case "erc20":
        return await _erc20Balance(chain, addr);
      case "erc20_polygon":
        return await _erc20PolygonBalance(chain, addr);
      case "sol":
        return await _solBalance(addr);
      case "trx":
        return await _trxBalance(addr);
      case "xlm":
        return await _xlmBalance(addr);
      case "xrp":
        return await _xrpBalance(addr);
      case "ltc":
        return await _ltcBalance(addr);
      case "electrum":
        return await _electrumBalance(chain, addr);
      case "xmr":
        return _xmrBalance();
      default:
        return { balance: 0, loaded: false };
...
function _persistBalances(results) {
  let prev = {};
  try {
    prev = JSON.parse(localStorage.getItem("00_balances") || "{}");
  } catch {
  }
...
window.chainsGetPrices = async function() {
  ...
  try {
    ...
    console.warn("[chains] Kraken price fetch failed:", e.message);
  }
  try {
    ...
    console.warn("[chains] CryptoCompare price fetch failed:", e.message);
  }
  if (!out.usdc) out.usdc = { price: 1, change24h: 0 };
  if (!out.usdt) out.usdt = { price: 1, change24h: 0 };
  try {
    localStorage.setItem(ck, JSON.stringify({ data: out, _ts: Date.now() }));
  } catch {
  }
  return out;
};
async function _trxHistory(addr, limit) {
  ...
  return j.data.map((tx) => {
    ...
    return {
      ...
      timestamp: Math.floor((tx.block_timestamp || 0) / 1e3)
    };
...
async function _xlmHistory(addr, limit) {
  ...
  return records.filter((op) => op.type === "payment" || op.type === "create_account").map((op) => ({
    ...
    timestamp: Math.floor(Date.parse(op.created_at) / 1e3)
  }));
}
...
async function _solHistory(addr, limit) {
  ...
  try {
    ...
    const txs = [];
    for (const sig of sigs.slice(0, Math.min(limit, 10))) {
      try {
  ...
  return new Promise((resolve) => {
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        const txs = transactions.filter((t) => t.tx?.TransactionType === "Payment").map((t) => ({
          txid: t.tx.hash,
          chain: "xrp",
...
window.chainsGetHistory = async function(chain, addr, limit = 20) {
  ...
  try {
    switch (chain) {
      case "trx":
        return await _trxHistory(addr, limit);
      case "xlm":
        return await _xlmHistory(addr, limit);
      case "ltc":
        return await _ltcHistory(addr, limit);
      case "eth":
      case "bnb":
      case "avax":
      case "matic":
        return await _evmHistory(chain, addr, limit);
      case "sol":
...
window.chainsPersistHistory = function(chain, txs) {
  let existing = [];
  try {
    existing = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
  } catch {
  }

...
import { balanceChipHtml, updateBalanceChip } from "../core/ui-helpers.js";
const id = "dex";
const title = "00 DEX";
const icon = "\u25C8";
...
function fmtUsd(v) {
  ...
  if (v < 1e-3 && v > 0) return "$" + v.toExponential(2);
  return "$" + v.toFixed(v < 1 ? 4 : 2);
}
function fmtBp(bp) {
  return bp ? (bp / 100).toFixed(2) + "%" : "0.00%";
}
function tSym(t) {
  const s = t.bcmr?.token?.symbol || t.display_symbol || "";
  return s.length > 10 ? s.slice(0, 10) + "\u2026" : s || (t.token_id ? t.token_id.slice(0, 6) + "\u2026" : "???");
}
function tName(t) {
  const n = t.bcmr?.name || t.display_name || "";
  return n.length > 20 ? n.slice(0, 20) + "\u2026" : n || (t.token_id ? t.token_id.slice(0, 8) + "\u2026" : "Unknown");
}
function tIcon(t) {
  return META + "/icon/" + t.token_id;
}
function _template() {
  return `<div class="dt-inner" style="padding:32px 40px">
    ...
    <div class="dt-pane active" id="dt-dex-p-swap">
      ...
      <div class="dt-card">
        ...
        <div style="text-align:center;font-size:11px;color:var(--dt-text-secondary);margin-top:8px">Powered by Cauldron DEX \xB7 0.3% LP fee</div>
      </div>
    </div>
...
function _quote() {
  ...
  const fee = fromAmt * 3e-3;
  const toFinal = toAmt * (1 - 3e-3);
  ...
  document.getElementById("dt-swap-rate").innerHTML = "<div>Rate: <span>1 BCH \u2248 " + (rate >= 1 ? rate.toFixed(4) : rate.toExponential(4)) + " " + sym + "</span></div><div>LP Fee: <span>0.3% (" + fee.toFixed(4) + " BCH)</span></div><div>Source: <span>Cauldron AMM</span></div>";
}
...
async function _loadTokens() {
  try {
    ...
    console.warn("[dex] token load failed:", e);
  }
}
function _bind() {
  ...
  document.getElementById("dt-sort-price")?.addEventListener("click", () => {
    _sortKey = "price_now_usd";
    _renderTokens();
  });
  document.getElementById("dt-sort-change")?.addEventListener("click", () => {
    _sortKey = "change_24h_bp";
    _renderTokens();
  });
  document.getElementById("dt-sort-tvl")?.addEventListener("click", () => {
    _sortKey = "tvl_sats";
    _renderTokens();
  });
}

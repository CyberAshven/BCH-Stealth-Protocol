...
async function _lc(){if(_schnorr)return;const[c,h]=await Promise.all([import('../lib/noble-curves.js'),import('../lib/noble-hashes.js')]);_schnorr=c.schnorr;_sha256S=h.sha256;_b2h=b=>[...b].map(x=>x.toString(16).padStart(2,'0')).join('');_h2b=h=>new Uint8Array(h.match(/.{2}/g).map(x=>parseInt(x,16)));_utf8=s=>new TextEncoder().encode(s);}
...
async function _qr(id,text,color){const c=document.getElementById(id);if(!c||!text)return;if(!_QRLib){const m=await import('../lib/qrcode.js');_QRLib=m.default||m;}await _QRLib.toCanvas(c,text,{width:200,margin:1,color:{dark:color||'#0AC18E',light:'#ffffff'}});}

...
function pk(id2) {
  if (id2 === "sbch") return "bch";
  if (id2 === "matic") return "matic";
  if (id2 === "usdce_polygon") return "usdc";
  if (id2.startsWith("usdc")) return "usdc";
  if (id2.startsWith("usdt")) return "usdt";
  if (id2.startsWith("rlusd")) return "usdc";
  return id2;
}
function getC(id2) {
...
function renderCoinList() {
  ...
  const cards = CHAINS.map((c) => {
    ...
    const ico = c.iconType === "img" ? `<img class="wd-acc-icon" src="${c.icon}" alt="${c.ticker}">` : `<div class="wd-acc-icon" style="background:${c.color}"><span>${c.icon}</span></div>`;
...
function renderCoinDetail(coinId) {
  ...
  const ico = c.iconType === "img" ? `<img src="${c.icon}" alt="${c.ticker}" style="width:48px;height:48px;border-radius:50%">` : `<div style="width:48px;height:48px;border-radius:50%;background:${c.color};display:flex;align-items:center;justify-content:center"><span style="color:#fff;font-size:22px;font-weight:700">${c.icon}</span></div>`;
  ...
  <div style="padding:24px 40px 40px" class="v2-coin-detail">
    <div class="cd-head">
      ...
      <div class="cd-actions">
        ${coinId === "sbch" ? '<button class="cd-action-btn" id="cd-scan-stealth-btn" style="border-color:#7c3aed;color:#7c3aed"><span>\u{1F50D}</span> Scan</button>' : ""}
    ...
    <div class="cd-tx-card">
      ...
      ${coinId === "bch" || coinId === "sbch" ? `<div id="cd-stealth-panel" style="display:none;padding:20px">
        <div style="margin-bottom:16px">
          <div style="font-size:13px;font-weight:600;color:var(--dt-text);margin-bottom:4px">\u{1F512} Stealth Scanner</div>
        ...
        <div style="display:flex;gap:8px;margin-bottom:12px">
          <button id="cd-stealth-scan" style="flex:1;padding:10px;border:none;border-radius:8px;background:var(--dt-accent,#0AC18E);color:#fff;font-weight:600;cursor:pointer;font-size:13px">\u{1F50D} Scan Range</button>
        ...
        <div style="display:flex;gap:8px;margin-bottom:16px;align-items:center">
          ...
          <button id="cd-stealth-check-tx" style="padding:10px 16px;border:none;border-radius:8px;background:#7c3aed;color:#fff;font-weight:600;cursor:pointer;font-size:12px;white-space:nowrap">\u{1F50E} Check TX</button>
        </div>
      ...
      <div class="cd-modal">
        ...
        <div id="cd-recv-normal" ${coinId === "sbch" ? 'style="display:none"' : ""}>
          ...
          <div class="cd-copy-row">
            <button class="cd-copy-btn" id="cd-copy-full" style="background:var(--dt-accent);color:#fff;border-color:var(--dt-accent)">\u{1F4CB} COPY FULL</button>
        ...
        ${coinId === "bch" || coinId === "sbch" ? `<div id="cd-recv-stealth" style="${coinId === "sbch" ? "" : "display:none"}">
          ...
          <div class="cd-copy-row">
            <button class="cd-copy-btn" id="cd-copy-stealth" style="background:#BF5AF2;color:#fff;border-color:#BF5AF2">\u{1F4CB} COPY STEALTH CODE</button>
          </div>
          <div class="cd-path-info">// BIP352 STEALTH \xB7 SCAN + SPEND PUBKEYS</div>
        </div>` : ""}
      </div>
    </div>
  </div>`;
  ...
  document.getElementById("cd-max-btn")?.addEventListener("click", async () => {
    ...
    if (evmTokens.includes(coinId)) {
      try {
        ...
        if (token && addr) {
          const bal2 = await polyTx.checkBalance(token, addr);
          amtEl.value = (Number(bal2) / Math.pow(10, c2.dec)).toFixed(c2.dec <= 6 ? 2 : 6);
        }
      } catch {
        const balances2 = state.get("balances") || {};
        amtEl.value = (Number(balances2[coinId] || 0) / Math.pow(10, c2.dec)).toFixed(c2.dec <= 6 ? 2 : 6);
      }
      ...
      try {
        ...
        if (addr) {
          ...
          const bal2 = BigInt(balHex || "0x0");
          const gasPrice = BigInt(await _rpc("eth_gasPrice") || "0x0");
          const gasCost = gasPrice * 21000n * 2n;
          const maxWei = bal2 > gasCost ? bal2 - gasCost : 0n;
          amtEl.value = (Number(maxWei) / 1e18).toFixed(6);
        }
      } catch {
        const balances2 = state.get("balances") || {};
        amtEl.value = (Number(balances2[coinId] || 0) / 1e18).toFixed(6);
      }
    } else if (coinId === "trx" || coinId === "usdt_trx") {
      const balances2 = state.get("balances") || {};
      const bal2 = Number(balances2[coinId] || 0);
      const reserve = coinId === "trx" ? 1e6 : 0;
      const max = Math.max(0, bal2 - reserve);
      amtEl.value = (max / 1e6).toFixed(6);
    } else if (coinId === "xlm") {
      const balances2 = state.get("balances") || {};
      const bal2 = Number(balances2[coinId] || 0);
      const reserve = 1.5 * 1e7;
      const maxStroops = Math.max(0, bal2 - reserve - 100);
      amtEl.value = (maxStroops / 1e7).toFixed(7);
    } else if (coinId === "xrp") {
      const balances2 = state.get("balances") || {};
      const bal2 = Number(balances2[coinId] || 0);
      const reserve = 10 * 1e6;
      const maxDrops = Math.max(0, bal2 - reserve - 12);
      amtEl.value = (maxDrops / 1e6).toFixed(6);
    } else if (coinId === "sol") {
      const balances2 = state.get("balances") || {};
      const bal2 = Number(balances2[coinId] || 0);
      const fee = 5e3;
      const maxLamports = Math.max(0, bal2 - fee);
  ...
  document.getElementById("cd-stealth-max")?.addEventListener("click", async () => {
    const balances2 = state.get("balances") || {};
    const bal2 = balances2[coinId] || 0;
    const amtEl = document.getElementById("cd-stealth-amount");
    const fee = Math.ceil((10 + 148 + 34) * 1);
    if (amtEl && bal2 > 0) amtEl.value = (Math.max(0, bal2 - fee) / 1e8).toFixed(8);
  });
  document.getElementById("send-mode-normal")?.addEventListener("click", () => {
    ...
    const n2 = document.getElementById("cd-send-normal");
    if (n2) n2.style.display = "flex";
    const s = document.getElementById("cd-send-stealth");
  ...
  document.getElementById("recv-mode-normal")?.addEventListener("click", () => {
    ...
    const n2 = document.getElementById("cd-recv-normal");
    if (n2) n2.style.display = "";
    const s = document.getElementById("cd-recv-stealth");
    if (s) s.style.display = "none";
  });
  ...
  document.querySelectorAll(".cd-dir-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".cd-dir-btn").forEach((b) => {
        b.classList.remove("active");
        b.style.background = "transparent";
        b.style.color = "var(--dt-text-secondary)";
      });
      btn.classList.add("active");
      btn.style.background = "var(--dt-accent)";
      btn.style.color = "#fff";
  ...
  document.getElementById("cd-page-size")?.addEventListener("change", (e) => {
    _txPageSize = parseInt(e.target.value);
    _loadTransactions(coinId);
  });
}
...
async function _loadTransactions(coinId) {
  ...
  try {
    const c2 = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
    hasCachedTxs = c2.some((t) => t.chain === chain);
  } catch {
  }
  if (!hasCachedTxs) {
  ...
  try {
    const keys = auth.getKeys();
    if (!keys) {
      el.innerHTML = '<div class="cd-tx-empty">Not connected</div>';
      return;
    }
    let allTxs = [];
    const chain2 = coinId === "sbch" ? "bch" : coinId;
    try {
      const cached = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
      allTxs = cached.filter((tx) => {
        if (tx.chain !== chain2) return false;
    ...
    const cacheKey = "00_tx_scan_ts_" + chain2;
    ...
    if (needsScan && (coinId === "bch" || coinId === "sbch") && window._fvCall && keys.bchAddr) {
      try {
            ...
            try {
              ...
              if (header && header.length >= 152) {
                ...
                for (const tx of allTxs) {
                  if (tx.height === ht && !tx.timestamp) tx.timestamp = ts;
                }
              }
            } catch {
            }
        ...
        console.warn("[wallet] live tx enrich:", e);
      }
    }
    ...
    if (needsScan && evmChains.includes(coinId) && !allTxs.length) {
      try {
        ...
        console.warn("[wallet] EVM tx fetch:", e);
      }
    }
        ...
        try {
          ...
          for (const tx of unconfirmed) {
            if (tx.height > 0) {
              ...
              if (idx >= 0) {
                hist[idx].height = tx.height;
                if (tx.timestamp) hist[idx].timestamp = tx.timestamp;
              }
            }
          }
    ...
    if ((coinId === "bch" || coinId === "sbch") && window._fvCall) {
      ...
      try {
        const h = await window._btcCall("blockchain.headers.subscribe", []);
        tipHeight = h?.height || 0;
      } catch {
      }
    }
    ...
    const counterText = _txFilter === "all" ? `Showing ${showing} of ${totalCount} transactions` : `Showing ${showing} of ${filteredCount} ${_txFilter === "in" ? "received" : "sent"} (${totalCount} total)`;
    ...
    for (const tx of txs) {
      ...
      const tsMs = rawTs > 0 ? rawTs * 1e3 : now;
      ...
      const onclickAttr = isFusionRow ? `window._openFusionDetail && window._openFusionDetail('${tx.txid}')` : `window.open('${explorerBase}${tx.txid}','_blank')`;
...
async function _doStealthCheckTx(coinId) {
  ...
  if (!txid || txid.length < 60) {
    if (statusEl) statusEl.textContent = "Enter a valid txid";
    return;
  }
      ...
      for (const f of found) {
        try {
          ...
          if (!hist.some((h) => h.txid === f.txid)) {
            hist.unshift({ txid: f.txid, chain: "bch", dir: "stealth", amount: f.value, height: f.height || 0, timestamp: Math.floor(Date.now() / 1e3) });
...
async function _populateCoinControl(coinId) {
  ...
  try {
    ...
    if (!utxos.length) {
      list.innerHTML = '<div style="font-size:11px;text-align:center;padding:8px">No UTXOs found</div>';
      return;
    }
    ...
    list.innerHTML = '<div style="font-size:11px;text-align:center;padding:8px;color:#ef4444">Error loading UTXOs</div>';
  }
}
async function _fetchUtxos(coinId) {
  ...
  if (coinId === "btc" && window._btcCall) {
    ...
    const script = new Uint8Array([118, 169, 20, ...decoded.slice(1), 136, 172]);
    const hash = sha(script);
    const sh = Array.from(hash).reverse().map((b) => b.toString(16).padStart(2, "0")).join("");
...
function _getSelectedUtxos() {
  ...
  if (!toggle?.checked || !window._ccUtxos) return null;
...
async function _doSendEvm(coinId) {
  ...
  if (!to || !to.startsWith("0x") || to.length !== 42) {
    if (errEl) errEl.textContent = "Enter a valid 0x address";
    return;
  }
  if (amt <= 0) {
  ...
  try {
    ...
    if (tokenAddr) {
      ...
      const data = "0x" + selector + to.slice(2).toLowerCase().padStart(64, "0") + amountRaw.toString(16).padStart(64, "0");
      txHash = await polyTx.signAndSend(privKey, tokenAddr, data, "0x0", 1e5, chainOpts);
    } else {
      const amountWei = BigInt(Math.floor(amt * 1e18));
      txHash = await polyTx.signAndSend(privKey, to, "0x", "0x" + amountWei.toString(16), 21e3, chainOpts);
    }
    if (btn) btn.textContent = "Confirming...";
    const receipt = await polyTx.waitForTx(txHash, 3e4, chainOpts);
    if (receipt.status === 1) {
      if (errEl) {
        errEl.textContent = "";
        errEl.style.color = "";
...
async function _doSendLegacy(coinId) {
  ...
  const amt = parseFloat(document.getElementById("cd-send-amount")?.value) || 0;
  if (!addr) {
    if (errEl) errEl.textContent = "Recipient address required";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
  ...
  if (amtSats < 546) {
    if (errEl) errEl.textContent = "Minimum 546 sats (dust limit)";
    return;
  }
  const activeFee = document.querySelector(".cd-fee-opt.active");
  ...
  if (!keys?.acctPriv || !keys?.acctChain) {
    if (errEl) errEl.textContent = "HD wallet required for " + coinId.toUpperCase();
    return;
  }
  const { bip32Child } = await import("../core/hd.js");
  const { secp256k1: secp } = await import("../lib/noble-curves.js");
  const chainIndex = coinId === "btc" ? 3 : 6;
  ...
  try {
    ...
    try {
      let hist = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
      hist.unshift({ txid: result.txid, chain: coinId, dir: "out", amount: amtSats, height: 0, timestamp: Math.floor(Date.now() / 1e3) });
...
async function _doSendTrx(coinId) {
  ...
  if (!to || !to.startsWith("T") || to.length !== 34) {
    if (errEl) errEl.textContent = "Enter a valid T... address";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
...
async function _doSendXrp(coinId) {
  ...
  const to = document.getElementById("cd-send-addr")?.value.trim();
  const amt = parseFloat(document.getElementById("cd-send-amount")?.value) || 0;
  if (!to || !to.startsWith("r") || to.length < 25 || to.length > 35) {
    if (errEl) errEl.textContent = "Enter a valid r... XRP address";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
  ...
  try {
    ...
    const destinationTag = document.getElementById("cd-send-memo")?.value?.trim() || void 0;
...
async function _doSendXlm(coinId) {
  ...
  if (!to || !to.startsWith("G") || to.length !== 56) {
    if (errEl) errEl.textContent = "Enter a valid G... Stellar address";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
  ...
  try {
    ...
    const memo = document.getElementById("cd-send-memo")?.value?.trim() || void 0;
...
async function _doSendSol(coinId) {
  ...
  const to = document.getElementById("cd-send-addr")?.value.trim();
  const amt = parseFloat(document.getElementById("cd-send-amount")?.value) || 0;
  if (!to || to.length < 32 || to.length > 44) {
    if (errEl) errEl.textContent = "Enter a valid Solana address";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
...
async function _doSendXmr() {
  ...
  if (!addr.startsWith("4") && !addr.startsWith("8")) {
    if (errEl) errEl.textContent = "Invalid XMR address (must start with 4 or 8)";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
...
async function _doSendBch(coinId) {
  ...
  const keys = auth.getKeys();
  if (!keys) {
    if (errEl) errEl.textContent = "Wallet not unlocked";
    return;
  }
  let utxos = [];
  const hdAddrs = state.get("hdAddresses") || [];
  ...
  if (!utxos.length) {
    if (errEl) errEl.textContent = "No UTXOs \u2014 refresh balance";
    return;
  }
  _sending = true;
  if (btn) {
  ...
  try {
    ...
    if (keys.ledger) {
      ...
      if (!changeHash160) {
        ...
        changeHash160 = ca2h(keys.bchAddr);
      }
    } else if (changeAddr) {
      const { cashAddrToHash20: ca2h } = await import("../core/cashaddr.js");
    ...
    try {
      let hist = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
      hist.unshift({ txid: result.txid, chain: "bch", dir: "out", amount: amtSats, height: 0, timestamp: Math.floor(Date.now() / 1e3) });
      localStorage.setItem("00_tx_history", JSON.stringify(hist.slice(0, 500)));
    } catch {
    }
    if (btn) {
...
async function _doStealthSpend() {
  ...
  const amtSats = Math.round(amt * 1e8);
  if (amtSats < 546) {
    if (errEl) errEl.textContent = "Minimum 546 sats";
    return;
  }
  _sending = true;
  if (btn) {
  ...
  try {
    ...
    for (const su of saved) {
      ...
      stealthKeyMap[su.addr] = typeof su.priv === "string" ? new Uint8Array(su.priv.match(/.{2}/g).map((b) => parseInt(b, 16))) : su.priv;
      try {
        const h = cashAddrToHash20(su.addr);
        const script = new Uint8Array([118, 169, 20, ...h, 136, 172]);
        const hash = sha(script);
    ...
    try {
      let hist = JSON.parse(localStorage.getItem("00_tx_history") || "[]");
      hist.unshift({ txid: result.txid, chain: "bch", dir: "stealth", amount: -amtSats, height: 0, timestamp: Math.floor(Date.now() / 1e3) });
      localStorage.setItem("00_tx_history", JSON.stringify(hist.slice(0, 500)));
    } catch {
    }
    if (btn) {
...
async function _doStealthSend(coinId) {
  ...
  if (!code) {
    if (errEl) errEl.textContent = "Stealth code required";
    return;
  }
  if (amt <= 0) {
    if (errEl) errEl.textContent = "Enter a valid amount";
...
async function _loadUtxos(coinId) {
  ...
  try {
    let renderSection = function(title2, icon2, color, utxoList) {
      ...
      let h = `<div style="display:flex;justify-content:space-between;align-items:center;padding:16px 24px;border-bottom:1px solid var(--dt-border);background:var(--dt-bg)">
        <div style="display:flex;align-items:center;gap:8px">
          <span style="color:${color};font-size:16px">${icon2}</span>
          <span style="font-size:14px;font-weight:700;color:var(--dt-text);letter-spacing:.5px">${title2}</span>
      ...
      try {
        ...
        for (const su of saved) {
          ...
          try {
            ...
            console.warn("[wallet] stealth UTXO fetch error:", e2);
          }
        }
      } catch {
      }
...
window._openFusionDetail = async function(txid) {
  ...
  const outputRows = outputs ? outputs.map((o) => `
    ...
    </div>`).join("") : `<div style="font-size:12px;color:var(--dt-text-secondary);text-align:center;padding:12px">Output details not available</div>`;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            ...
            <div style="font-size:11px;color:var(--dt-text-secondary)">${peers} peer${peers > 1 ? "s" : ""} \xB7 ${height > 0 ? "Block #" + height : "Unconfirmed"}</div>
          </div>
        </div>
        <button id="_wv-fus-close" style="background:transparent;border:none;color:var(--dt-text-secondary);font-size:22px;cursor:pointer;line-height:1;padding:4px">\xD7</button>
      </div>
...
function _walletPromptPassword(title2 = "Enter your wallet password") {
  return new Promise((resolve, reject) => {
    ...
    el.innerHTML = `<div style="background:var(--dt-surface,#16171f);border:1px solid var(--dt-border,#232430);border-radius:16px;padding:28px 32px;min-width:320px;max-width:420px;width:90%;box-shadow:var(--dt-shadow-lg,0 4px 16px rgba(0,0,0,.3))">
      <div style="font-size:15px;font-weight:700;margin-bottom:16px;color:var(--dt-text,#e2e8f0)">${title2}</div>
    ...
    el.querySelector("#_wv-pass-cancel").onclick = () => {
      cleanup();
...
window.openExportKeys = async function() {
  try {
    ...
    if (vault) {
      try {
        await auth.decryptVault(vault, pass);
      } catch {
    ...
    try {
      if (keys.acctPriv && keys.acctChain) {
        let _childPriv = function(par, chain, idx) {
          const d = new Uint8Array(37);
          d[0] = 0;
          d.set(par, 1);
          new DataView(d.buffer).setUint32(33, idx >>> 0, false);
          const h = hmac(sha512, chain, d);
          const l = h.slice(0, 32), r = h.slice(32);
          const t = (BigInt("0x" + b2h2(l)) + BigInt("0x" + b2h2(par))) % N;
          const k = new Uint8Array(32);
          const ts = t.toString(16).padStart(64, "0");
        ...
        const b2h2 = (arr) => Array.from(arr).map((b) => b.toString(16).padStart(2, "0")).join("");
        const N = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141n;
        const btcNode = _childPriv(keys.acctPriv, keys.acctChain, 3);
        btcPrivHex = b2h2(btcNode.priv);
      }
    } catch {
    }
    if (keys.xmr) {
    ...
    m.innerHTML = `<div style="background:var(--dt-surface,#16171f);border:1px solid var(--dt-border,#232430);border-radius:16px;padding:24px;width:100%;max-width:600px;max-height:90vh;overflow-y:auto;box-shadow:var(--dt-shadow-lg,0 4px 16px rgba(0,0,0,.3))">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div style="font-size:16px;font-weight:700;color:var(--dt-text,#e2e8f0)">\u{1F511} Export Keys</div>
        <button id="_wv-keys-close" style="background:transparent;border:none;color:var(--dt-text,#e2e8f0);font-size:20px;cursor:pointer;line-height:1">\xD7</button>
      </div>
    ...
    if (e.message !== "cancelled") console.warn("[wallet] openExportKeys:", e);
  }
};
window.exportBackup = async function() {
  try {
    ...
    if (e.message !== "cancelled") {
      console.warn("[wallet] exportBackup:", e);
...
window.importBackup = async function(file) {
  ...
  try {
    ...
    if (e.message !== "cancelled") {
      console.warn("[wallet] importBackup:", e);
      alert("Import failed: " + e.message);
    }
  }
};
function mount(container, subRoute) {

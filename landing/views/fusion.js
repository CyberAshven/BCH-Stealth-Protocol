...
const id = "fusion";
const title = "00 Fusion";
const icon = "\u2697";
const NOSTR_KIND_POOL = 22230;
const NOSTR_KIND_JOINER = 22231;
const PHASE_TIMEOUT = 6e4;
const MIN_PARTICIPANTS = 2;
const MIN_MIX_SATS = 3e3;
const POOL_WAIT_MIN = 3e4;
const POOL_WAIT_MAX = 12e4;
const POOL_PEER_TTL = 3e5;
...
async function addrToScriptHash(addr) {
  ...
  const script = new Uint8Array([118, 169, 20, ...raw, 136, 172]);
...
function _saveFusionStealthUtxo(addr, priv, pub) {
  ...
  existing.push({ addr, priv: b2h(priv), pub: b2h(pub), from: "fusion", ts: Math.floor(Date.now() / 1e3) });
    ...
    const results = await Promise.all(addrs.map(async (addr) => {
      try {
        ...
        const us = await Promise.race([
          ...
          new Promise((r) => setTimeout(() => r(null), 1e4))
        ]);
...
function _destroyRoundIdentity() {
  if (_roundPriv) {
    for (let i = 0; i < _roundPriv.length; i++) _roundPriv[i] = 0;
  }
  _roundPriv = null;
  _roundPub = null;
  _unsubscribeRound();
...
function _checkPoolThreshold() {
  ...
  if (statusEl) statusEl.textContent = "STARTING IN " + Math.round(waitMs / 1e3) + "s...";
...
async function _handleGiftWrap(ev) {
  ...
  try {
    ...
    console.warn("[FUSION] unwrap error:", e);
  }
}
...
async function _signalReady() {
  ...
  const id2 = b2h(idHash);
  const sig = b2h(await schnorr.sign(idHash, _roundPriv));
  const ev = { id: id2, pubkey: _roundPub, created_at, kind: NOSTR_KIND_POOL, tags, content, sig };
  _nostrPublish(ev);
  _setPhase(1);
  _checkPoolThreshold();
}
...
async function _startInputRegistration() {
  ...
  if (usable.length === 0) {
    showToast("No spendable UTXOs", "error");
    _resetMix();
    return;
  }
  ...
  if (outputTotal < 546 * nOutputs) {
    showToast("Balance too low for mix", "error");
    _resetMix();
    return;
  }
  const amounts = randomSplit(outputTotal, nOutputs);
  if (!amounts) {
    showToast("Cannot split balance", "error");
    _resetMix();
    return;
  }
...
async function _startTxAssembly() {
  ...
  try {
    ...
    const allOutputs = m.blindOutputs.filter((o) => o.value >= 546).map((o) => ({ value: o.value, script: p2pkhAddrScript(o.addr) }));
...
async function _startBroadcast() {
  ...
  const signedInputs = m.allInputs.map((inp, i) => {
    for (const sigs of Object.values(m.signatures)) {
      const s = sigs.find((s2) => s2.index === i);
  ...
  try {
    ...
    if (p6) {
      ...
      p6.classList.add("done");
    }
    const myTotal = m.myOutputs.reduce((s, o) => s + o.value, 0);
    _mixHistory.unshift({
      txid,
      amount: satsToBch(myTotal),
      peers: m.peers.length,
      outputs: m.myOutputs.length,
      time: Date.now(),
      status: "ok"
    });
    ...
    try {
      ...
      console.warn("[FUSION] could not save to tx_history:", e);
    }
    for (const out of m.myOutputs) {
      if (out.stealth) {
    ...
    _balanceSats = _utxos.reduce((s, u) => s + u.value, 0);
    state.merge("balances", { bch: _balanceSats });
    setTimeout(_refreshBalance, 5e3);
...
async function _handlePhaseTimeout(phase) {
  ...
  _phaseError(
    phase,
    blamed.length > 0 ? "Timeout - " + blamed.length + " peer(s) dropped" : "Timeout - round aborted"
  );
  _mixHistory.unshift({
    txid: "TIMEOUT",
    amount: "0",
...
function _resetMix() {
  ...
  document.querySelectorAll("#dt-mix-phases .dt-phase-item").forEach(
    (el) => el.classList.remove("active", "done", "error")
  );
...
async function _autoMixNext() {
  if (!_autoMix) return;
  if (_activeMix) {
    _autoMixStatus("Waiting for current round...");
    return;
  }
  if (_balanceSats < MIN_MIX_SATS) {
...
function _autoMixScheduleRetry() {
  ...
  _autoMixStatus("Retrying in " + Math.round(delay / 1e3) + "s...");
...
function _renderPool() {
  ...
  el.innerHTML = `<div style="font-size:12px;font-weight:600;color:var(--dt-text-secondary);margin-bottom:8px">${total} peer${total > 1 ? "s" : ""} in pool${_poolTimer ? " \u2014 round starting soon..." : ""}</div>` + peers.map((p) => `
    <div class="dt-row">
...
function _renderHistory() {
  ...
  if (!_mixHistory.length) {
    el.innerHTML = '<div class="dt-empty"><div class="dt-empty-icon">\u{1F4CB}</div><div class="dt-empty-text">No mix history yet</div></div>';
    return;
  }
  el.innerHTML = _mixHistory.slice(0, 20).map((h) => {
    const amt = h.amount || "?";
    const outs = h.outputs ? " \xB7 " + h.outputs + " outputs" : "";
    const isOk = h.status === "ok";
    const badge = isOk ? '<span style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:6px;border:1px solid var(--dt-accent-border,#0AC18E);color:var(--dt-accent,#0AC18E)">Mixed</span>' : '<span style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:6px;border:1px solid var(--dt-danger-border,#E53935);color:var(--dt-danger,#E53935)">Failed</span>';
    return `<div class="dt-row" style="cursor:default">
      <div class="dt-row-left">
        ...
        <div>
          ...
          <div class="dt-row-sub">${h.txid?.slice(0, 16) || "\u2014"}... \xB7 ${h.peers || "?"} participants${outs}</div>
        </div>
      </div>
...
function _template() {
  return `<div class="dt-inner" style="padding:32px 40px">
    <div class="dt-page-header">
      <div class="dt-page-title-wrap">
        ...
        <div>
          ...
          <div class="dt-page-sub">Silent Joiner \xB7 Privacy Mixing</div>
        </div>
      </div>
    ...
    ([n, label, sub]) => `<div class="dt-phase-item" data-phase="${n}" style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--dt-border)">
    ...
    <div class="dt-pane" id="dt-fus-p-history">
      <div id="dt-fus-history">
        <div class="dt-empty"><div class="dt-empty-icon">\u{1F4CB}</div><div class="dt-empty-text">No mix history yet</div></div>
      </div>
    </div>
  </div>
...
async function mount(container) {
  ...
  try {
    _mixHistory = JSON.parse(localStorage.getItem("00_fusion_history") || "[]");
  } catch {
    _mixHistory = [];
  }
  _renderHistory();

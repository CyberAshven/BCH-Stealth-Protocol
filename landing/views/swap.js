...
const MAKER_TIMEOUT = 144;
const TAKER_TIMEOUT = 72;
const P2SH_VBYTES = 280;
const OFFER_TTL = 1800;
const MIN_BCH_SATS = 1e3;
...
let _activePair = "bch_btc";
let _offers = [], _myOffers = /* @__PURE__ */ new Set(), _seenEvents = /* @__PURE__ */ new Set();
...
function _hashToCashAddr(hash20, vb) {
  const p = new Uint8Array([vb, ...hash20]);
  const d5 = [];
  let acc = 0, bits = 0;
  for (const b of p) {
    acc = acc << 8 | b;
...
function pubHashToCashAddr(h) {
  return _hashToCashAddr(h, 0);
}
function scriptHashToCashAddr(h) {
  return _hashToCashAddr(h, 8);
}
const B58 = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
function base58Check(payload) {
  const inp = concat(payload, dsha256(payload).slice(0, 4));
  let n = BigInt("0x" + b2h(inp));
  let s = "";
  while (n > 0n) {
...
function base58Decode(str) {
  ...
  for (const c of str) {
    ...
    if (i === -1) throw new Error("bad b58");
    n = n * 58n + BigInt(i);
  }
...
function btcP2pkhAddr(pub33) {
  return base58Check(concat(new Uint8Array([0]), ripemd160(sha256(pub33))));
}
function ltcP2pkhAddr(pub33) {
  return base58Check(concat(new Uint8Array([48]), ripemd160(sha256(pub33))));
}
function p2pkhScript(h20) {
  return concat(new Uint8Array([118, 169, 20]), h20, new Uint8Array([136, 172]));
}
function p2shScript(rs) {
  return concat(new Uint8Array([169, 20]), ripemd160(sha256(rs)), new Uint8Array([135]));
}
function pushData(d) {
  if (d.length <= 75) return concat(new Uint8Array([d.length]), d);
  if (d.length <= 255) return concat(new Uint8Array([76, d.length]), d);
  return concat(new Uint8Array([77, d.length & 255, d.length >> 8 & 255]), d);
}
...
function htlcScript(hash32, recipPub33, senderPub33, locktime) {
  ...
  return concat(
    new Uint8Array([99, 168, 32]),
    hash32,
    new Uint8Array([136, 33]),
    recipPub33,
    new Uint8Array([172, 103]),
    new Uint8Array([lt.length]),
    lt,
    new Uint8Array([177, 117, 33]),
    senderPub33,
    new Uint8Array([172, 104])
  );
}
function serializeTx(ver, lt, ins, outs) {
  return concat(
    ...
    ...outs.flatMap((o) => [u64LE(o.value), writeVarint(o.script.length), o.script]),
    u32LE(lt)
  );
}
function bchSighash(ver, lt, ins, outs, i, utxoScript, utxoVal) {
  ...
  const inp = ins[i];
  return dsha256(concat(u32LE(ver), dsha256(po), dsha256(sq), inp.txidLE, u32LE(inp.vout), writeVarint(utxoScript.length), utxoScript, u64LE(utxoVal), u32LE(inp.sequence), dsha256(od), u32LE(lt), u32LE(65)));
}
function btcSighash(ver, lt, ins, outs, i, subscript) {
  ...
  for (const o of outs) parts.push(u64LE(o.value), writeVarint(o.script.length), o.script);
  parts.push(u32LE(lt), u32LE(1));
...
function buildBchHtlcFundTx(utxos, redeemScript, amtSats, changeAddr) {
  ...
  for (const u of sorted) {
    sel.push(u);
    total += u.value;
    const fee2 = Math.ceil((10 + sel.length * 148 + 2 * 34) * 1);
    if (total >= amtSats + fee2) break;
  }
  ...
  const ins = sel.map((u) => ({ txidLE: h2b(u.txid).reverse(), vout: u.vout, value: u.value, sequence: 4294967295, scriptSig: new Uint8Array(0) }));
  for (let i = 0; i < ins.length; i++) {
    ...
    const der = concat(sig.toDERRawBytes(), new Uint8Array([65]));
...
function buildBchClaimTx(txid, vout, value, redeemScript, preimage, recipPriv, outAddr) {
  const fee = Math.ceil((10 + P2SH_VBYTES + 34) * 1);
  const outVal = value - fee;
  if (outVal < 546) throw new Error("htlc too small");
  const pub = secp256k1.getPublicKey(recipPriv, true);
  const ins = [{ txidLE: h2b(txid).reverse(), vout, value, sequence: 4294967295, scriptSig: new Uint8Array(0) }];
  ...
  const der = concat(sig.toDERRawBytes(), new Uint8Array([65]));
  ins[0].scriptSig = concat(new Uint8Array([der.length]), der, new Uint8Array([preimage.length]), preimage, new Uint8Array([81]), pushData(redeemScript));
  return b2h(serializeTx(1, 0, ins, outs));
}
function buildBchRefundTx(txid, vout, value, redeemScript, locktime, senderPriv, outAddr) {
  ...
  const outVal = value - fee;
  if (outVal < 546) throw new Error("htlc too small");
  const ins = [{ txidLE: h2b(txid).reverse(), vout, value, sequence: 4294967294, scriptSig: new Uint8Array(0) }];
  const outs = [{ value: outVal, script: p2pkhScript(cashAddrToHash20(outAddr)) }];
  const sh = bchSighash(1, locktime, ins, outs, 0, redeemScript, value);
  const sig = secp256k1.sign(sh, senderPriv, { lowS: true });
  const der = concat(sig.toDERRawBytes(), new Uint8Array([65]));
  ins[0].scriptSig = concat(new Uint8Array([der.length]), der, new Uint8Array([0]), pushData(redeemScript));
...
function buildBtcHtlcFundTx(utxos, privKey, redeemScript, amtSats, changeAddr, feeRate) {
  ...
  for (const u of sorted) {
    sel.push(u);
    total += u.value;
    const fee2 = Math.ceil((10 + sel.length * 148 + 2 * 34) * feeRate);
    if (total >= amtSats + fee2) break;
  }
  ...
  for (let i = 0; i < ins.length; i++) {
    ...
    const der = concat(sig.toDERRawBytes(), new Uint8Array([1]));
    ins[i].scriptSig = concat(new Uint8Array([der.length]), der, new Uint8Array([pub.length]), pub);
  }
  return b2h(serializeTx(1, 0, ins, outs));
}
function buildBtcClaimTx(txid, vout, value, redeemScript, preimage, recipPriv, outAddr, feeRate) {
  const fee = Math.ceil((10 + P2SH_VBYTES + 34) * feeRate);
  const outVal = value - fee;
  if (outVal < 546) throw new Error("htlc too small");
  const ins = [{ txidLE: h2b(txid).reverse(), vout, value, sequence: 4294967295, scriptSig: new Uint8Array(0) }];
...
function _readVarint(buf, off) {
  const b = buf[off];
  if (b < 253) return { value: b, next: off + 1 };
  if (b === 253) return { value: buf[off + 1] | buf[off + 2] << 8, next: off + 3 };
  return { value: (buf[off + 1] | buf[off + 2] << 8 | buf[off + 3] << 16 | buf[off + 4] << 24) >>> 0, next: off + 5 };
}
function _parseTxInputs(hex) {
  try {
    const b = h2b(hex);
    let p = 4;
    const ic = _readVarint(b, p);
    p = ic.next;
    const ins = [];
    for (let i = 0; i < ic.value; i++) {
      const txid = b2h(b.slice(p, p + 32).reverse());
      p += 32;
      const vout = (b[p] | b[p + 1] << 8 | b[p + 2] << 16 | b[p + 3] << 24) >>> 0;
      p += 4;
      const sl = _readVarint(b, p);
      p = sl.next;
      const ss = b2h(b.slice(p, p + sl.value));
      p += sl.value;
      p += 4;
      ins.push({ txid, vout, scriptSig: ss });
    }
    return ins;
  } catch {
    return [];
...
function _parseTxOutputs(hex) {
  ...
  const oc = _readVarint(b, p);
  p = oc.next;
  const outs = [];
  for (let i = 0; i < oc.value; i++) {
    let v = 0;
    for (let j = 0; j < 8; j++) v += b[p + j] * 256 ** j;
    p += 8;
    const sl = _readVarint(b, p);
    p = sl.next;
    outs.push({ value: v, script: b.slice(p, p + sl.value) });
    p += sl.value;
  }
  return outs;
}
...
function _bchAddrSH(addr) {
  return b2h(sha256(p2pkhScript(cashAddrToHash20(addr))).reverse());
}
function _btcAddrSH(addr) {
  return b2h(sha256(p2pkhScript(base58Decode(addr))).reverse());
}
function _p2shSH(rs) {
  return b2h(sha256(p2shScript(rs)).reverse());
}
function _fv(method, params) {
  return window._fvCall(method, params);
}
function _btcCall(method, params) {
  return window._btcCall ? window._btcCall(method, params) : Promise.reject("no btc");
}
function _chainCall(ch) {
  return ch === "bch" ? _fv : _btcCall;
}
...
function _loadHistory() {
  try {
    _swapHistory = JSON.parse(localStorage.getItem("00_swap_history_v2") || "[]");
  } catch {
    _swapHistory = [];
  }
}
...
function _bind() {
  ...
  if (bchIn) bchIn.addEventListener("input", () => {
    _lastEdited = "bch";
    _updateRate();
  });
  if (otherIn) otherIn.addEventListener("input", () => {
    _lastEdited = "other";
    _updateRate();
  });
...
function _subscribeNostr() {
  ...
  const since = Math.floor(Date.now() / 1e3) - OFFER_TTL;
...
async function _handleSwapEvent(ev) {
  ...
  switch (ev.kind) {
    ...
    case NOSTR_KIND_TAKE:
      await _handleTake(ev);
      break;
    case NOSTR_KIND_LOCKED:
      await _handleLocked(ev);
      break;
    case NOSTR_KIND_CLAIMED:
      await _handleClaimed(ev);
      break;
  }
}
...
function _handleOffer(ev) {
  try {
    ...
    if (idx >= 0) {
      ...
      if (_myOffers.has(_offers[idx].id)) {
        _myOffers.delete(_offers[idx].id);
        _myOffers.add(ev.id);
      }
      _offers[idx] = o;
    } else {
...
async function _postOffer() {
  const keys = _getKeys();
  if (!keys) {
    _toast("Wallet not unlocked", "error");
    return;
  }
  ...
  if (bchAmt < MIN_BCH_SATS) {
    if (statusEl) statusEl.textContent = "Min BCH: " + satsToBch(MIN_BCH_SATS);
    return;
  }
  if (otherAmt < MIN_BTC_SATS) {
    if (statusEl) statusEl.textContent = "Min amount: " + satsToBch(MIN_BTC_SATS);
    return;
  }
  const offer = { pair: _activePair, side, bch_amt: bchAmt, other_amt: otherAmt, btc_amt: otherAmt, maker_pub: sessionPub, stealth_code: keys.stealthCode || "", ts: Math.floor(Date.now() / 1e3) };
...
function _cancelOffer(id2) {
  _myOffers.delete(id2);
  _offers = _offers.filter((o) => o.id !== id2);
  _renderOrderbook();
...
async function _takeOffer(offerId) {
  ...
  if (!offer) {
    _toast("Offer not found", "error");
    return;
  }
  if (_swap) {
    _toast("Swap already active", "error");
    return;
  }
  const keys = _getKeys();
  const sessionPriv = keys.sessionPriv;
  ...
  if (offer.bch_amt < MIN_BCH_SATS) {
    _toast("BCH amount too small", "error");
    return;
  }
  if (oAmt < MIN_BTC_SATS) {
    _toast("Amount too small", "error");
    return;
  }
  ...
  _swap = {
    ...
    H_btc: b2h(H_btc),
    state: "TAKEN",
    steps: ["Take Sent", "Waiting Maker Lock", "Verifying HTLC", "Locking My HTLC", "Claiming", "Complete"],
    currentStep: 1,
    ts: Date.now()
  };
  _saveSwap();
...
async function _handleTake(ev) {
  ...
  try {
    ...
    if (offer.side === "sell_bch") {
      lockChain = "bch";
      lockAmt = offer.bch_amt;
      lockHash = take.H_bch;
      claimChain = chain;
      claimAmt = oAmt;
      claimHash = take.H_btc;
    } else {
      lockChain = chain;
      lockAmt = oAmt;
      lockHash = take.H_btc;
      claimChain = "bch";
      claimAmt = offer.bch_amt;
      claimHash = take.H_bch;
    }
    ...
    if (lockChain === "bch") {
      ...
      _toast("BTC lock via SPA not yet supported", "error");
      return;
    }
    ...
    console.error("[swap] handleTake:", e);
  }
}
async function _handleLocked(ev) {
  ...
  try {
    ...
    if (locked.offer_id !== _swap.offer.id) {
      _swapBusy = false;
      return;
    }
    const sessionPub = b2h(secp256k1.getPublicKey(sessionPriv, true).slice(1));
    ...
    if (!vr.ok) {
      _toast("HTLC verify failed: " + vr.reason, "error");
    ...
    _swap.claimRedeemScriptHex = b2h(claimRS);
    ...
    console.error("[swap] handleLocked:", e);
    _toast("Swap error: " + e.message, "error");
  } finally {
    _swapBusy = false;
  }
}
async function _handleClaimed(ev) {
  ...
  try {
    ...
    if (claimed.offer_id !== _swap.offer.id) {
      _swapBusy = false;
      return;
    }
    const sessionPub = b2h(secp256k1.getPublicKey(sessionPriv, true).slice(1));
    ...
    for (const inp of txIns) {
      ...
      if (pi && pi.length === 32 && b2h(sha256(pi)) === _swap.lockHash) {
        revealed = pi;
        break;
      }
    }
    if (!revealed) {
      _toast("Preimage extraction failed", "error");
      return;
    }
    const otherPre = _deriveOtherPreimage(revealed, sessionPriv, _swap.takerPub);
    if (b2h(sha256(otherPre)) !== _swap.claimHash) {
      _toast("Preimage derivation mismatch", "error");
      return;
    }
    _swap.revealedPreimage = b2h(revealed);
    _swap.otherPreimage = b2h(otherPre);
    _swap.currentStep = 4;
    ...
    console.error("[swap] handleClaimed:", e);
    _toast("Claim error: " + e.message, "error");
  } finally {
    _swapBusy = false;
  }
}
async function _refundSwap() {
  if (!_swap || !_swap.myHtlcTxid || !_swap.redeemScript) {
    _toast("Nothing to refund", "error");
    return;
  }
  const keys = _getKeys();
  try {
    ...
    if (curH < _swap.locktime) {
      _toast("Timelock not expired (" + (_swap.locktime - curH) + " blocks left)", "error");
      return;
    }
    ...
    if (utxos.length === 0) {
      _toast("HTLC output not found", "error");
      return;
    }
    const u = utxos[0];
    let tx;
    ...
    _toast("Refund error: " + e.message, "error");
  }
}
...
function _startMonitoring() {
  ...
  _monitorInterval = setInterval(_monitorSwap, 15e3);
}
  ...
  try {
    if (_swap.role === "taker" && (_swap.state === "WAITING_CONFIRM" || _swap.state === "TAKER_LOCKED") && _swap.claimRedeemScriptHex && !_swap.claimTxid) {
      ...
      if (utxos.length > 0) {
        ...
        _nostrPublish(ev);
        _swap.claimTxid = txid;
        _swap.state = "COMPLETE";
        _swap.currentStep = 6;
        _saveSwap();
        _renderSwapState();
        _toast("Swap complete! (recovered)", "success");
      }
    }
    if (_swap.role === "maker" && _swap.otherPreimage && !_swap.myClaimTxid) {
      ...
      if (_swap.takerLocktime) {
        ...
        if (utxos.length > 0) {
          ...
          _saveSwap();
          _renderSwapState();
          _toast("Swap complete! (maker recovered)", "success");
        }
      }
    }
  } catch (e) {
    console.error("[swap] monitor:", e);
  }
}
  ...
  el.innerHTML = pairOffers.sort((a, b) => b.ts - a.ts).map((o) => {
    ...
    return `<div class="dt-row" style="cursor:pointer">
      <div class="dt-row-left">
        ...
        <div class="dt-row-sub">Rate: 1 BCH = ${rate} ${chain} \xB7 ${agoStr} \xB7 ${pub8}...</div></div>
      </div>
      <div style="display:flex;align-items:center;gap:12px">
        ...
        ${isMine ? `<button class="dt-action-btn-outline" style="width:auto;padding:6px 14px;font-size:11px;color:var(--dt-danger);border-color:var(--dt-danger)" data-cancel="${o.id}">Cancel</button>` : `<button class="dt-action-btn" style="width:auto;padding:6px 14px;font-size:11px" data-take="${o.id}">Swap</button>`}
      </div>
    </div>`;
  }).join("");
...
function _renderSwapState() {
  ...
  if (actEl) {
    ...
    actEl.querySelector("#sw-clear-btn")?.addEventListener("click", () => {
      _swap = null;
      _saveSwap();
      _renderSwapState();
    });
  ...
  el.innerHTML = _swapHistory.slice().reverse().map((h) => {
    ...
    return `<div class="dt-row" style="cursor:default">
      <div class="dt-row-left"><div class="dt-row-icon in">\u21C4</div><div>
        ...
        <div class="dt-row-sub">${h.role} \xB7 ${dateStr}</div>
      </div></div>
...
function mount(container) {
  ...
  _unsubs.push(state.subscribe("balances", () => {
    updateBalanceChip("bch");
    updateBalanceChip("btc");
  }));
  ...
  setTimeout(_renderOrderbook, 1e3);
}
function unmount() {

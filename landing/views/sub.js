...
import { concat, b2h, h2b, dsha256, utf8, rand, satsToBch, showToast } from "../core/utils.js";
import { cashAddrToHash20, pubHashToCashAddr } from "../core/cashaddr.js";
import { p2pkhScript, bchSighash, serializeTx, estimateTxSize } from "../core/bch-tx.js";
import { bip32Child } from "../core/hd.js";
import { nostrSubscribe, nostrUnsubscribe, nostrPublish } from "../core/nostr-bridge.js";
const id = "sub";
const title = "00 Subscriptions";
const icon = "\u21BB";
...
let _subscriptions = [];
let _seenEvents = /* @__PURE__ */ new Set();
let _nostrSubIds = [];
...
function loadSubscriptions() {
  try {
    _subscriptions = JSON.parse(localStorage.getItem("00_subscriptions") || "[]");
  } catch {
    _subscriptions = [];
  }
}
...
function deriveSessionKeys(keys) {
  ...
  if (secp256k1.getPublicKey(_sessionPriv, true)[0] === 3) {
...
async function buildSubscriptionChain(amountSats, periods, recipAddr, intervalDays, label, peerPub) {
  ...
  const fundingFee = Math.ceil(estimateTxSize(
    selected.length,
...
async function cancelSubscription(subId) {
  ...
  if (broadcastTxs.length > 0) {
    ...
    if (!last.changeValue || last.changeValue < 546) {
      ...
      sub.chain_txs.forEach((t) => {
        if (t.status === "pending") t.status = "invalidated";
      });
      saveSubscriptions();
      _renderActive();
      return;
...
function startSettlementChecker() {
  ...
  _settlementInterval = setInterval(checkSubSettlements, 3e4);
  setTimeout(checkSubSettlements, 5e3);
}
function stopSettlementChecker() {
  if (_settlementInterval) {
    clearInterval(_settlementInterval);
    _settlementInterval = null;
  }
}
...
async function handleInvoiceEvent(ev) {
  try {
    ...
    for (const u of sorted) {
      selected.push(u);
      selectedTotal += u.value;
      if (selectedTotal >= amtSats + 1e3) break;
    }
    ...
    await fvCall("blockchain.transaction.broadcast", [rawHex]);
    sub.periods_paid++;
    sub.last_paid_ts = Math.floor(Date.now() / 1e3);
...
function startNostrListeners() {
  ...
  nostrSubscribe(
    ...
    if (subId) _nostrSubIds.push(subId);
  });
  nostrSubscribe(
...
function getNextPaymentDate(sub) {
  if (sub.type === "fixed_bch") {
    ...
    const d = new Date(nextTx.locktime * 1e3);
    return d.toLocaleDateString("en", { month: "short", day: "numeric", year: "numeric" });
  } else {
    if (!sub.next_due_ts) return "\u2014";
    const d = new Date(sub.next_due_ts * 1e3);
    return d.toLocaleDateString("en", { month: "short", day: "numeric", year: "numeric" });
  }
}
function _template() {
  return `<div class="dt-inner" style="padding:32px 40px">
    <div class="dt-page-header">
      <div class="dt-page-title-wrap"><div class="dt-page-icon"><img src="icons/sub.png" style="width:28px;height:28px"></div><div><div class="dt-page-title">Subscriptions</div><div class="dt-page-sub">BCH Recurring Payments \xB7 Trustless</div></div></div>
    ...
    <div class="dt-pane" id="dt-sub-p-history">
      <div id="dt-sub-history-list"><div class="dt-empty"><div class="dt-empty-icon">\u{1F4CB}</div><div class="dt-empty-text">No subscription history</div></div></div>
    </div>
  </div>`;
}
function _renderActive() {
  ...
  el.innerHTML = active.map((s) => {
    ...
    const roleTag = s.role === "receiver" ? '<span style="background:rgba(76,175,80,.12);color:#4caf50;padding:2px 8px;border-radius:4px;font-size:10px;margin-left:8px">RECEIVING</span>' : "";
    return '<div class="dt-card" style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div><span style="font-weight:700;color:var(--dt-text)">' + _esc(s.label || "Subscription") + "</span>" + roleTag + '</div><span style="color:var(--dt-text-secondary);font-size:12px">' + amtStr + " / " + s.interval_days + 'd</span></div><div style="background:var(--dt-border);border-radius:4px;height:6px;margin-bottom:8px;overflow:hidden"><div style="width:' + pct + '%;height:100%;background:var(--dt-accent);border-radius:4px;transition:width .3s"></div></div><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;color:var(--dt-text-secondary)">' + s.periods_paid + "/" + s.total_periods + " paid \xB7 Next: " + nextDate + "</span>" + (s.role === "payer" && s.type === "fixed_bch" ? '<button class="dt-action-btn-outline dt-sub-cancel-btn" style="width:auto;padding:4px 12px;font-size:11px;color:var(--dt-error,#ff4040);border-color:var(--dt-error,#ff4040)" data-subid="' + s.id + '">Cancel</button>' : "") + "</div></div>";
  }).join("");
...
function _renderIncoming() {
  ...
  el.innerHTML = incoming.map((s) => {
    const pct = s.total_periods > 0 ? Math.round(s.periods_paid / s.total_periods * 100) : 0;
    return '<div style="padding:12px;background:var(--dt-bg,#f5f6f8);border:1px solid var(--dt-border);border-radius:8px;margin-bottom:8px"><div style="font-weight:700;color:var(--dt-text);margin-bottom:4px">' + _esc(s.label || "Subscription") + '</div><div style="font-size:11px;color:var(--dt-text-secondary)">' + s.amount.toLocaleString() + " sats \xB7 " + s.periods_paid + "/" + s.total_periods + " paid (" + pct + "%)</div></div>";
  }).join("");
}
function _renderHistory() {
  ...
  if (!history.length) {
    el.innerHTML = '<div class="dt-empty"><div class="dt-empty-icon">\u{1F4CB}</div><div class="dt-empty-text">No subscription history</div></div>';
    return;
  }
  el.innerHTML = history.map((s) => {
    const statusBadge = s.status === "completed" ? '<span style="color:#4caf50;font-size:10px;font-weight:700">COMPLETED</span>' : '<span style="color:var(--dt-error,#ff4040);font-size:10px;font-weight:700">CANCELLED</span>';
    return '<div style="padding:12px;background:var(--dt-bg,#f5f6f8);border:1px solid var(--dt-border);border-radius:8px;margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:700;color:var(--dt-text)">' + _esc(s.label || "Subscription") + "</span>" + statusBadge + '</div><div style="font-size:11px;color:var(--dt-text-secondary);margin-top:4px">' + s.periods_paid + "/" + s.total_periods + " paid \xB7 " + s.amount.toLocaleString() + " sats/" + s.interval_days + "d</div></div>";
  }).join("");
}
function _esc(str) {
...
function _bind() {
    ...
    try {
      ...
      if (_subMode === "bch") {
        ...
        const sub = {
          ...
          peer_pub: peerPub,
          last_paid_ts: 0,
          next_due_ts: Math.floor(Date.now() / 1e3) + interval * 86400,
          auto_approve: true,
          periods_paid: 0,
          status: "active",
          created_at: Math.floor(Date.now() / 1e3)
  ...
  document.getElementById("dt-sub-gen-link")?.addEventListener("click", () => {
    ...
    const link = window.location.origin + "/sub.html?amount=" + amount + "&interval=" + interval + "&periods=" + periods + "&addr=" + encodeURIComponent(keys2?.bchAddr || "") + "&pub=" + (_sessionPub || "") + "&mode=bch";
    const display = document.getElementById("dt-sub-link-display");
    if (display) {
      display.style.display = "block";
      display.innerHTML = '<div style="margin-bottom:8px;color:var(--dt-text);font-weight:700">Subscription Link:</div><div style="word-break:break-all;font-size:11px;color:var(--dt-text-secondary);margin-bottom:12px">' + _esc(link) + '</div><button class="dt-action-btn-outline" id="dt-sub-copy-link" style="width:auto;padding:6px 14px;font-size:11px">Copy Link</button>';
      document.getElementById("dt-sub-copy-link")?.addEventListener("click", async () => {
        ...
        if (b) {
          b.textContent = "Copied \u2713";
          setTimeout(() => b.textContent = "Copy Link", 2e3);
        }
      });
    }
  });
...
function mount(container) {
  ...
  refreshUtxos();
  _balanceInterval = setInterval(refreshUtxos, 3e4);
}
function unmount() {

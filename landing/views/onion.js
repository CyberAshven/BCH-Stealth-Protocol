...
import { infoBtn, updateBalanceChip } from "../core/ui-helpers.js";
import { onionPeel, onionWrap, onionUnpad, makeEvent, nip04Decrypt, b2h, h2b } from "../onion-crypto.js";
import { deriveStealthSendAddr, decodeStealthCode } from "../core/stealth.js";
import { sendBch } from "../core/send-bch.js";
const id = "onion";
const title = "00 Onion";
const icon = "\u29C9";
const NOSTR_KIND_RELAY_ANN = 22230;
const NOSTR_KIND_ONION_BLOB = 22231;
const RELAY_TTL = 300;
const ANNOUNCE_INTERVAL = 6e4;
let _container = null;
let _unsubs = [];
let _knownRelays = [];
...
function _template() {
  ...
  return `<div class="dt-inner" style="padding:32px 40px;max-width:720px">
    <div class="dt-page-header">
      <div class="dt-page-title-wrap"><div class="dt-page-icon"><img src="icons/onion.png" style="width:28px;height:28px"></div><div><div class="dt-page-title">Onion Relay</div><div class="dt-page-sub">Privacy Routing \xB7 Stealth Addresses</div></div></div>
    </div>
    ...
    <div class="dt-pane active" id="dt-on-p-send">
      ...
      <div style="background:var(--dt-surface,#fff);border:1px solid var(--dt-border);border-radius:14px;padding:24px;margin-bottom:20px">
        ...
        <button class="dt-action-btn" id="dt-on-send-btn" style="background:linear-gradient(135deg,#BF5AF2,#8B5CF6);border:none">\u{1F9C5} Send via Stealth \u2192</button>
      ...
      <div style="background:var(--dt-surface,#fff);border:1px solid var(--dt-border);border-radius:14px;padding:24px">
        ...
        <div id="dt-on-relays"><div style="text-align:center;padding:24px;color:var(--dt-text-secondary);font-size:13px"><div style="font-size:24px;margin-bottom:8px;opacity:.4">\u{1F9C5}</div>Scanning for relays...</div></div>
      </div>
    </div>
...
function _pruneStaleRelays() {
  const cutoff = Math.floor(Date.now() / 1e3) - RELAY_TTL;
...
function _handleRelayAnnouncement(ev) {
  ...
  try {
    ...
    if (typeof info.fee !== "number" || info.fee < 0 || info.fee > 1e6) return;
    if (typeof info.max_htlc !== "number" || info.max_htlc < 546) return;
    const relay = {
      pub: ev.pubkey,
      fee: info.fee || 500,
      max_htlc: info.max_htlc || 5e6,
...
function _renderRelayList() {
  ...
  if (_knownRelays.length === 0) {
    el.innerHTML = '<div style="text-align:center;padding:24px;color:var(--dt-text-secondary);font-size:13px"><div style="font-size:24px;margin-bottom:8px;opacity:.4">\u{1F9C5}</div>No relays found yet</div>';
    return;
  }
  ...
  el.innerHTML = _knownRelays.map((r) => {
    ...
    return `<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid var(--dt-border);${!ok ? "opacity:.4;" : ""}">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:var(--dt-text-secondary);margin-top:2px">MAX: ${satsToBch(r.max_htlc)} BCH \xB7 CLTV: ${r.cltv_delta} \xB7 ${ago}</div>
      </div>
    ...
    if (data.type === "onion") {
      if (_relayMode && data.blob) {
        try {
          ...
          console.error("[ONION] peel error:", e);
        }
      }
    }
  } catch (e) {
    console.error("[ONION] blob error:", e);
  }
}
async function _subscribeNostr() {
  if (!window._nostrSubscribe) {
    console.warn("[ONION] Nostr bridge not ready");
    return;
  }
  const keys = auth.getKeys();
  const now = Math.floor(Date.now() / 1e3);
  try {
    ...
    console.error("[ONION] relay sub error:", e);
  }
  if (keys?.sessionPub) {
    const myPubHex = typeof keys.sessionPub === "string" ? keys.sessionPub : b2h(keys.sessionPub);
    try {
      ...
      console.error("[ONION] blob sub error:", e);
    }
  }
}
function _unsubscribeNostr() {
  if (window._nostrUnsubscribe) {
    if (_relaySub) {
      window._nostrUnsubscribe(_relaySub);
      _relaySub = null;
    }
    if (_blobSub) {
      window._nostrUnsubscribe(_blobSub);
      _blobSub = null;
    }
  }
}
function _updateNostrStatus() {
  ...
  if (label) {
    label.textContent = st.connected ? "Nostr: " + st.relayCount + " relay" + (st.relayCount !== 1 ? "s" : "") : "Nostr: disconnected";
  }
}
...
function _updateRelayToggleUI() {
  ...
  if (sts) {
    sts.textContent = _relayMode ? "Relay active \u2014 announcing on Nostr relays every 60 seconds." : "Relay not active. Toggle the switch above to start announcing on Nostr.";
  }
}
async function _announceRelay() {
  const keys = auth.getKeys();
  if (!keys?.sessionPriv) {
    console.warn("[ONION] no session key for relay announce");
    return;
  }
  if (!window._nostrPublish) {
    console.warn("[ONION] Nostr bridge not available");
    return;
  }
  ...
  const info = {
    ...
    max_htlc: parseInt(maxEl?.value) || 5e6,
  ...
  try {
    ...
    console.error("[ONION] announce error:", e);
  }
}
...
async function _handleStealthSend() {
  const keys = auth.getKeys();
  if (!keys) {
    _setStatus("Wallet not unlocked", true);
    return;
  }
  ...
  if (!recipientInput) {
    _setStatus("Enter a recipient address or stealth code", true);
    return;
  }
  ...
  if (amountSats < 546) {
    _setStatus("Amount below dust limit (546 sats)", true);
    return;
  }
  ...
  try {
    ...
    if (!toAddress) {
      _setStatus("Could not determine destination address", true);
      return;
    }
    if (useOnion && _knownRelays.length > 0) {
      const aliveRelays = _knownRelays.filter((r) => Math.floor(Date.now() / 1e3) - r.ts < RELAY_TTL);
      if (aliveRelays.length === 0) {
        _setStatus("No live relays available for onion routing", true);
        return;
      }
    ...
    if (!window._fvCall) {
      _setStatus("Fulcrum not connected", true);
      return;
    }
    try {
...
function _bindClipboard() {
  ...
  if (addrEl && keys?.bchAddr) {
    addrEl.addEventListener("click", () => {
      ...
      setTimeout(() => {
        addrEl.style.borderColor = "transparent";
      }, 1200);
    });
  }
  if (pubEl) {
    ...
    if (pub) {
      pubEl.addEventListener("click", () => {
        ...
        setTimeout(() => {
          pubEl.style.borderColor = "transparent";
        }, 1200);
      });
    }
  }
  if (stealthEl && keys?.stealthCode) {
    stealthEl.addEventListener("click", () => {
      ...
      setTimeout(() => {
        stealthEl.style.borderColor = "rgba(191,90,242,.15)";
      }, 1200);
    });
  }
}
function mount(container) {
  ...
  _updateNostrStatus();
  _renderTimer = setInterval(_renderRelayList, 3e4);
  _pruneTimer = setInterval(_pruneStaleRelays, 6e4);
  if (_relayMode) {
    _announceRelay();
    _relayAnnTimer = setInterval(_announceRelay, ANNOUNCE_INTERVAL);
  }

...
function fmtUsd(n) {
  ...
  if (n >= 1e4) return "$" + n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
function getTimeLeftMs(tf) {
  ...
  const end = new Date(start.getTime() + tfObj.minutes * 60 * 1e3);
...
function _connectBinance() {
  ...
  try {
    ...
    _binanceWs.onclose = () => {
      _binanceWs = null;
      setTimeout(_connectBinance, 5e3);
    };
    _binanceWs.onerror = () => {
...
async function fetchMarket(asset, tf) {
  ...
  const ts = Math.floor(period.getTime() / 1e3);
  ...
  try {
    ...
    try {
      outcomes = typeof mkt.outcomes === "string" ? JSON.parse(mkt.outcomes) : mkt.outcomes || [];
    } catch {
      outcomes = [];
    }
    try {
      outPrices = typeof mkt.outcomePrices === "string" ? JSON.parse(mkt.outcomePrices) : mkt.outcomePrices || [];
    } catch {
      outPrices = [];
    }
    try {
    ...
    const spread = 1 - (upPrice + downPrice);
...
function _patchCard(assetId) {
  ...
  if (spreadEl) {
    spreadEl.textContent = "Spread: " + (mkt.spread * 100).toFixed(1) + "%";
    spreadEl.style.color = mkt.spread > 1e-3 ? "#10B981" : "";
  }
  if (volEl) volEl.textContent = "Vol: $" + (mkt.volume >= 1e3 ? (mkt.volume / 1e3).toFixed(0) + "K" : mkt.volume.toFixed(0));
...
async function fetchAllMarkets() {
  try {
    ...
    for (const ev of events) {
      ...
      try {
        tokenIds = typeof mkt.clobTokenIds === "string" ? JSON.parse(mkt.clobTokenIds) : mkt.clobTokenIds || [];
      } catch {
      }
...
function _renderMain() {
  ...
  const tabBar = tabs.map(
    (t) => `<button data-action="tab" data-tab="${t.id}"
    ...
    <div class="dt-page-header">
      <div class="dt-page-title-wrap">
        ...
        <div>
          ...
          <div class="dt-page-sub">Polymarket \xB7 Crypto Markets</div>
        </div>
      </div>
    </div>
...
function _htmlMarkets() {
  ...
  for (const asset of ASSETS) {
    ...
    const spreadClass = spread > 1e-3 ? "color:#10B981;font-weight:700" : "color:var(--dt-text-secondary,#64748b)";
    const hasArb = spread > 5e-3;
    cards += `
  ...
  return `
    ...
    <div style="text-align:center;margin-top:12px;font-size:10px;color:var(--dt-text-secondary,#64748b)">
      Markets auto-refresh every 10s \xB7 Prices via Binance WebSocket
    </div>`;
}
...
function _updateCountdowns() {
  ...
  if (_activeTab === "positions") {
    ...
    for (const el of allCountdowns) {
      ...
      const endMs = Number(end) > 1e12 ? Number(end) : Number(end) > 1e9 ? Number(end) * 1e3 : new Date(end).getTime();
      ...
      if (ms <= 0) {
        ...
        el.style.color = ms < 12e4 ? "#EF4444" : "var(--dt-accent,#0AC18E)";
        el.style.animation = ms < 12e4 ? "pulse-red 1s infinite" : "";
      }
    }
  }
}
function _htmlPositions() {
  ...
  if (_ordersLoading) {
    ...
    for (const o of _openOrders) {
      ...
      <div class="bet-table-row" style="grid-template-columns:2.2fr 0.6fr 0.8fr 0.7fr 0.8fr 0.7fr 1fr 0.7fr">
        ...
        <div style="font-weight:600">${priceCents}\xA2</div>
  ...
  if (_positionsLoading) {
    ...
    for (const p of _positions) {
      ...
      const endTs = slugTs > 1e9 ? (slugTs + tfMinutes * 60) * 1e3 : p.endDate ? (/* @__PURE__ */ new Date(p.endDate + "T23:59:59")).getTime() : 0;
      ...
      <div style="display:grid;grid-template-columns:0.6fr 2fr 0.6fr 0.7fr 0.6fr 0.6fr 1fr 0.8fr 1fr;gap:4px;padding:10px 12px;font-size:12px;border-bottom:1px solid ${rowBorder};background:${rowBg};align-items:center">
        ...
        <div>${(p.avgPrice * 100).toFixed(1)}\xA2</div>
        <div style="font-weight:600">${(p.curPrice * 100).toFixed(1)}\xA2</div>
...
function _htmlAllMarkets() {
  const catBar = CATEGORIES.map(
    (c) => `<button class="bet-cat-btn${_activeCat === c.id ? " active" : ""}" data-action="category" data-cat="${c.id}">
      ${c.icon} ${c.label}
    </button>`
  ).join("");
  ...
  if (filtered.length === 0) {
    ...
    rows = filtered.slice(0, 50).map((m) => {
      ...
      <div class="bet-mkt-row" data-action="open-market" data-slug="${m.slug || m.id}">
        ${m.image ? `<img src="${m.image}" style="width:44px;height:44px;border-radius:10px;object-fit:cover;flex-shrink:0" onerror="this.style.display='none'">` : `<div style="width:44px;height:44px;border-radius:10px;background:#8247E5;display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;flex-shrink:0;font-weight:700">P</div>`}
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:10px;color:var(--dt-text-secondary,#64748b);margin-top:3px">
            Vol: ${fmtVol(m.volume)}${endStr ? " \xB7 Ends " + endStr : ""}
          </div>
        </div>
        <div style="display:flex;gap:12px;align-items:center;flex-shrink:0">
          <div style="text-align:center">
            <div style="font-size:18px;font-weight:700;color:#10B981">${(yesPrice * 100).toFixed(0)}\xA2</div>
            <div style="font-size:9px;font-weight:600;color:var(--dt-text-secondary,#64748b)">YES</div>
          </div>
          <div style="text-align:center">
            <div style="font-size:18px;font-weight:700;color:#EF4444">${(noPrice * 100).toFixed(0)}\xA2</div>
  ...
  <div>${rows}</div>
  <div style="text-align:center;margin-top:12px;font-size:10px;color:var(--dt-text-secondary,#64748b)">
    Showing ${Math.min(filtered.length, 50)} of ${filtered.length} markets \xB7 Data from Gamma API
  </div>`;
}
...
async function _ensureClob() {
  ...
  const privHex = privKey instanceof Uint8Array || Array.isArray(privKey) ? Array.from(privKey).map((b) => b.toString(16).padStart(2, "0")).join("") : typeof privKey === "string" ? privKey.replace(/^0x/, "") : String(privKey);
...
async function _placeBet(assetId, direction) {
  const mkt = _marketsData[assetId + "-" + _activeTimeframe];
  if (!mkt) {
    _toast("No market data available", true);
    return;
  }
      ...
      <div style="background:#fff;border-radius:16px;padding:32px;width:420px;max-width:90vw;box-shadow:0 20px 60px rgba(0,0,0,.2)">
        ...
        <div style="background:#f8fafc;border-radius:10px;padding:16px;margin-bottom:16px">
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:8px">
            ...
            <span style="font-weight:600">${(price * 100).toFixed(1)}\xA2</span>
          </div>
  ...
  modal.querySelectorAll(".bet-quick-amt").forEach((b) => {
    b.onclick = () => {
      modal.querySelector("#bet-modal-amt").value = b.dataset.amt;
    };
  });
  modal.querySelector("#bet-modal-close").onclick = () => modal.remove();
  modal.querySelector('div[style*="position:fixed"]').onclick = (e) => {
    if (e.target === e.currentTarget) modal.remove();
  };
  function setBetStep(n, status) {
    const el = modal.querySelector("#bet-ms" + n);
    if (!el) return;
    const icon2 = el.querySelector(".bms-icon");
    const text = el.querySelector("span:last-child");
    if (status === "active") {
      icon2.style.background = "#FEF3C7";
      icon2.style.color = "#D97706";
      icon2.textContent = "\u23F3";
      text.style.color = "#D97706";
      text.style.fontWeight = "600";
    } else if (status === "done") {
      icon2.style.background = "#D1FAE5";
      icon2.style.color = "#10B981";
      icon2.textContent = "\u2713";
      text.style.color = "#10B981";
      text.style.fontWeight = "400";
    } else if (status === "error") {
      icon2.style.background = "#FEE2E2";
      icon2.style.color = "#EF4444";
      icon2.textContent = "\u2717";
  ...
  modal.querySelector("#bet-modal-confirm").onclick = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= 3; i++) {
        const icon2 = modal.querySelector("#bet-ms" + i + " .bms-icon");
        if (icon2?.textContent === "\u23F3") {
          setBetStep(i, "error");
...
function _buyUp(assetId) {
  _placeBet(assetId, "up");
}
function _buyDown(assetId) {
  _placeBet(assetId, "down");
}
function _buyArb(assetId) {
  const mkt = _marketsData[assetId + "-" + _activeTimeframe];
  if (!mkt || mkt.spread <= 5e-3) {
    _toast("No arbitrage opportunity", true);
    return;
  }
  const asset = ASSETS.find((a) => a.id === assetId);
...
async function _openBetModal(slug) {
  ...
  if (ev.isMulti) {
    try {
      ...
      if (r.ok) {
        ...
        if (full.markets && full.markets.length > 0) {
          ...
          for (const m of full.markets) {
            ...
            try {
              tids = typeof m.clobTokenIds === "string" ? JSON.parse(m.clobTokenIds) : m.clobTokenIds || [];
            } catch {
            }
            const yesIdx = 0;
      ...
      console.warn("[bet] lazy fetch failed:", e);
    }
  }
  const modal = document.createElement("div");
  ...
  for (let i = 0; i < allOutcomes.length; i++) {
    ...
    <div class="bet-modal-outcome" data-idx="${i}" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border:2px solid #e2e8f0;border-radius:10px;cursor:pointer;transition:all .15s;background:#fff">
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:18px;font-weight:700;color:#1a1a2e">${priceCents}\xA2</div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:16px">
          <div style="display:flex;gap:12px;align-items:center;flex:1;min-width:0">
            ...
            <div style="min-width:0">
              ...
              <div style="font-size:11px;color:#64748b;margin-top:2px">Vol: ${fmtVol(ev.volume)}${endStr ? " \xB7 Ends " + endStr : ""}</div>
            </div>
          </div>
  ...
  modal.querySelectorAll(".bet-modal-outcome").forEach((el) => {
    el.onclick = () => {
      ...
      modal.querySelector("#bet-sum-price").textContent = (selectedOutcome.price * 100).toFixed(1) + "\xA2";
  ...
  modal.querySelector("#bet-modal-confirm").onclick = async () => {
    if (!selectedOutcome || !selectedOutcome.tokenId) {
      _toast("Select an outcome first", true);
      return;
    }
    const amt = parseFloat(modal.querySelector("#bet-modal-amt").value);
    if (!amt || amt < 1) {
      _toast("Minimum bet is $1", true);
      return;
    }
    const confirmBtn = modal.querySelector("#bet-modal-confirm");
    confirmBtn.disabled = true;
    ...
    const negRisk = selectedOutcome.negRisk !== void 0 ? selectedOutcome.negRisk : ev.negRisk;
    try {
      setStep(1, "active");
      const clob = await _ensureClob();
      setStep(1, "done");
      ...
      for (let i = 1; i <= 3; i++) {
        const icon2 = modal.querySelector("#bm-s" + i + " .bms-icon");
        if (icon2?.textContent === "\u23F3") {
          setStep(i, "error");
...
async function _redeemAll() {
  ...
  if (redeemable.length === 0) {
    _toast("No redeemable positions", true);
    return;
  }
  ...
  try {
    ...
    for (const p of unique) {
      try {
        ...
        const data = "0x" + selector + USDCE.replace("0x", "").padStart(64, "0") + parentId + condId + "";
        _toast(`Redeeming ${redeemed + 1}/${unique.length}...`);
        const txHash = await polyTx.signAndSend(privHex, CTF, data, "0x0", 3e5);
    ...
    setTimeout(() => _fetchPositions(), 3e3);
  } catch (e) {
...
async function _cancelAllOrders() {
  if (_openOrders.length === 0) {
    _toast("No open orders to cancel", true);
    return;
  }
  const confirmed = await _showConfirmModal(
    "Cancel All Orders",
  ...
  for (const o of [..._openOrders]) {
    try {
      ...
      console.warn("[bet] cancel failed for", o.id, e);
    }
  }
...
function _htmlWallet() {
  ...
  <div style="max-width:700px">
    ...
    <div style="padding:20px;background:#fff;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:24px">
      ...
      <p style="font-size:12px;color:#64748b;margin-bottom:12px">Uniswap V3 \xB7 0.01% fee \xB7 Polygon</p>
...
async function _loadWalletData() {
  ...
  let polyTx;
  try {
    polyTx = await import("../core/polygon-tx.js");
  } catch (e) {
    console.warn("[bet] polygon-tx import failed:", e);
    return;
  }
  let addr = null;
  try {
  ...
  try {
    ...
    console.warn("[bet] balance fetch error:", e);
  }
...
function _setApprovalRow(idx, status) {
  ...
  const icon2 = _container?.querySelector(`#bet-appr-icon-${idx}`);
  ...
  if (icon2) icon2.textContent = s.icon;
  if (label) {
...
async function _doSwap() {
  ...
  modal.querySelector("#bet-swap-close-btn").onclick = () => {
    modal.remove();
    _loadWalletData();
  };
  modal.querySelector('div[style*="position:fixed"]').onclick = (e) => {
  ...
  function setStep(n, status) {
    ...
    if (!el) return;
    const icon2 = el.querySelector(".bet-step-icon");
    const text = el.querySelector("span:last-child");
    if (status === "active") {
      icon2.style.background = "#FEF3C7";
      icon2.style.color = "#D97706";
  ...
  try {
    ...
    if (allowance < amountRaw) {
      ...
      const approveTx = await polyTx.signAndSend(privKey, tokenIn, polyTx.encodeApprove(C.SWAP_ROUTER), "0x0", 6e4);
      await polyTx.waitForTx(approveTx);
    }
    setStep(1, "done");
    setStep(2, "active");
    const deadline = Math.floor(Date.now() / 1e3) + 300;
    const minOut = BigInt(Math.floor(Number(amountRaw) * 0.99));
    const feeTier = 100;
    const swapData = polyTx.encodeExactInputSingle(tokenIn, tokenOut, feeTier, addr, deadline, Number(amountRaw), Number(minOut));
    setStep(2, "done");
    setStep(3, "active");
    const swapTx = await polyTx.signAndSend(privKey, C.SWAP_ROUTER, swapData, "0x0", 2e5);
    setStep(3, "done");
    setStep(4, "active");
    ...
    for (let i = 1; i <= 4; i++) {
      const icon2 = modal.querySelector("#bet-swap-s" + i + " .bet-step-icon");
      if (icon2?.textContent === "\u23F3") {
        setStep(i, "error");
  ...
  try {
    ...
    for (let i = 0; i < erc20Approvals.length; i++) {
      ...
      if (a < 10n ** 12n) {
        ...
        count++;
        const tx = await polyTx.signAndSend(privKey, token, polyTx.encodeApprove(spender), "0x0", 1e5);
    ...
    for (let i = 0; i < ctfApprovals.length; i++) {
      ...
      if (!a) {
        ...
        count++;
        const tx = await polyTx.signAndSend(privKey, C.CTF, polyTx.encodeSetApprovalForAll(spender), "0x0", 1e5);
        await polyTx.waitForTx(tx);
  ...
  try {
    ...
    for (let i = 0; i < erc20Revokes.length; i++) {
      ...
      if (a > 0n) {
        ...
        const tx = await polyTx.signAndSend(privKey, token, data, "0x0", 6e4);
        await polyTx.waitForTx(tx);
    ...
    for (let i = 0; i < ctfRevokes.length; i++) {
      ...
      if (a) {
        ...
        count++;
        const tx = await polyTx.signAndSend(privKey, C.CTF, polyTx.encodeSetApprovalForAll(spender, false), "0x0", 1e5);
        await polyTx.waitForTx(tx);
...
function _showConfirmModal(title2, message, confirmText = "Confirm", confirmColor = "#EF4444") {
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:#fff;border-radius:16px;padding:28px;max-width:420px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.2)">
        <h3 style="font-size:18px;font-weight:700;margin-bottom:8px;color:#1a1a2e">${title2}</h3>
    ...
    overlay.querySelector("#_cm_cancel").addEventListener("click", () => {
      overlay.remove();
      resolve(false);
    });
    overlay.querySelector("#_cm_confirm").addEventListener("click", () => {
      overlay.remove();
      resolve(true);
    });
  });
...
function _createClickHandler() {
  return function(e) {
    ...
    if (action === "tab") _switchTab(btn.dataset.tab);
      ...
      if (srcEl && amt) {
        const v = srcEl.textContent.replace("$", "").trim();
        if (v !== "--") amt.value = v;
      }
...
function mount(container) {
  ...
  _balanceIv = setInterval(_fetchPolygonBalances, 3e4);
  fetchAllCurrentMarkets();
  _countdownIv = setInterval(_updateCountdowns, 1e3);
...
function unmount() {
  ...
  if (_refreshIv) {
    clearInterval(_refreshIv);
    _refreshIv = null;
  }
  if (_balanceIv) {
    clearInterval(_balanceIv);
    _balanceIv = null;
  }

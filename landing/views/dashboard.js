...
let _container = null;
let _unsubs = [];
let _wizWalletMgr = null;
let _wizConnectedDapp = null;
function _reqSeq(req) {
  if (!req) return null;
  return req.sequence ?? req.seq ?? null;
}
function _extractSignedTx(req) {
  if (!req) return "";
...
function _showWizSignModal(req, mgr) {
  ...
  title2.style.cssText = "font-weight:700;font-size:16px;margin-bottom:4px";
  title2.textContent = "Sign Request";
  ...
  [title2, from, pre, timer, btns].forEach((el) => box.appendChild(el));
...
async function _openWizardDappMode() {
  ...
  const makeTabBtn = (label, active) => {
    ...
    btn.style.cssText = "flex:1;padding:6px;border:none;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600;transition:background .15s;" + (active ? "background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.1)" : "background:transparent;color:#64748b");
  ...
  const dappInput = document.createElement("input");
  ...
  dappConnectBtn.onclick = () => {
    ...
    if (!uri) {
      dappStatus.textContent = "Paste a wiz:// URI first";
      return;
    }
    try {
      const dapp = new WC.DappManager("00 Wallet", "");
      dapp.onConnect((name, icon2, paths) => {
  ...
  genQrBtn.onclick = async () => {
    ...
    _wizWalletMgr.onConnect((name) => {
      _wizConnectedDapp = name;
      walletStatus.textContent = "Connected: " + name;
      _updateWizStatus();
    });
    _wizWalletMgr.onSignRequest((req) => {
      _showWizSignModal(req, _wizWalletMgr);
    });
    _wizWalletMgr.onDisconnect(() => {

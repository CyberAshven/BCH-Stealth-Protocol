...
async function _scanQrUri() {
  ...
  return await new Promise((resolve, reject) => {
    ...
    overlay.querySelector("#scan-close").onclick = () => {
      cleanup();
      resolve(null);
    };
...
async function _pasteUriModal() {
  return await new Promise((resolve) => {
    ...
    const input = overlay.querySelector("#scan-paste-input");
    input?.focus();
    overlay.querySelector("#scan-paste-cancel").onclick = () => {
      cleanup();
      resolve(null);
    };
    overlay.querySelector("#scan-paste-ok").onclick = () => {
      const uri = (input?.value || "").trim();
      cleanup();
      resolve(uri || null);
    };

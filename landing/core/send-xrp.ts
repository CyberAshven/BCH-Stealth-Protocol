...
function xrpBase58Decode(addr: string): Uint8Array {
  let n = 0n;
...
/* ── Half-SHA512 ── */
function halfSha512(...parts: Uint8Array[]): Uint8Array { return sha512(concat(...parts)).slice(0, 32); }

/* ── XRPL Binary Field Encoding ── */
function fieldId(type: number, field: number): Uint8Array {
...
function encU16(type: number, field: number, v: number): Uint8Array { const b = new Uint8Array(2); b[0] = v >> 8; b[1] = v & 0xff; return concat(fieldId(type, field), b); }
function encU32(type: number, field: number, v: number): Uint8Array { const b = new Uint8Array(4); new DataView(b.buffer).setUint32(0, v >>> 0); return concat(fieldId(type, field), b); }

function encAmount(type: number, field: number, drops: number | bigint): Uint8Array {
...
function encVL(type: number, field: number, data: Uint8Array): Uint8Array {
...
function encAcct(type: number, field: number, accountId20: Uint8Array): Uint8Array { return encVL(type, field, accountId20); }

/* ── Serialize Payment TX (canonical field order) ── */
interface SerializeForSigningParams {
  srcAcctId: Uint8Array; dstAcctId: Uint8Array; amountDrops: number | bigint;
  feeDrops: number; sequence: number; lastLedgerSeq: number;
  pubKey33: Uint8Array; destinationTag?: number | string | null;
}
function serializeForSigning({ srcAcctId, dstAcctId, amountDrops, feeDrops, sequence, lastLedgerSeq, pubKey33, destinationTag }: SerializeForSigningParams): Uint8Array {
  ...
  if (destinationTag !== undefined && destinationTag !== null && destinationTag !== '') {
    parts.push(encU32(2, 14, parseInt(String(destinationTag)))); // DestinationTag
  }
  parts.push(
...
/* ── Send XRP ── */
interface SendXrpParams { toAddress: string; amountDrops: number; privKey32: Uint8Array; fromAddress: string; destinationTag?: number | string | null; }
export async function sendXrp({ toAddress, amountDrops, privKey32, fromAddress, destinationTag }: SendXrpParams): Promise<{ txid: string }> {
  ...
  const rpcUrl: string = (window as any)._00ep?.xrp_rpc || WS_URL;
...
/* ── WS one-shot ── */
function _wsCmd(url: string, cmd: object): Promise<any> {

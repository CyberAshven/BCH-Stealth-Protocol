...
function _hashToCashAddr(hash20: Uint8Array, versionByte: number, prefix = 'bitcoincash'): string {
  const payload = new Uint8Array([versionByte, ...hash20]);
  const d5: number[] = []; let acc = 0, bits = 0;
  ...
  const cs: number[] = []; for (let i = 7; i >= 0; i--) cs.push(Number((mod >> (BigInt(i) * 5n)) & 31n));
  return prefix + ':' + [...d5, ...cs].map(v => _caCharset[v]).join('');
}

export function pubHashToCashAddr(hash20: Uint8Array): string {
  return _hashToCashAddr(hash20, 0x00);
}

export function scriptHashToCashAddr(hash20: Uint8Array): string {
  return _hashToCashAddr(hash20, 0x08);
}

export function cashAddrToHash20(addr: string): Uint8Array {
  ...
  const bytes: number[] = []; let acc = 0, bits = 0;

...
function jacobianAdd(x1, y1, z1, x2, y2, z2) {
    ...
    if (u1 === u2)
        return s1 === s2 ? jacobianDouble(x1, y1, z1) : [0n, 1n, 0n];
...
function jacobianMul(k, px, py) {
    ...
    while (k > 0n) {
        if (k & 1n)
            [rx, ry, rz] = jacobianAdd(rx, ry, rz, qx, qy, qz);
...
function hTagInputs(data) { return taggedHash(TAG_H, data); }
function hTagShared(data) { return taggedHash(TAG_T, data); }
function hTagLabel(data) { return taggedHash(TAG_LABEL, data); }
// Reduce a 32-byte hash to a non-zero scalar mod N.
function hashToScalar(h) {
...
function labelTweakMod(scanPrivHex, m) {
    if (m === 0)
        return 0n;
...
function applyLabelToSpendPub(spendPubHex, tweak_m) {
    if (tweak_m === 0n)
        return spendPubHex;
...
function schnorrPubXOnly(privHex) {
    ...
    if (d0 === 0n)
        throw new Error('schnorr: zero priv');
...
function schnorrSign(msg, privHex, auxRand) {
    if (msg.length !== 32)
        throw new Error('schnorr: msg must be 32 bytes');
    let d0 = BigInt('0x' + privHex) % N;
    if (d0 === 0n || d0 >= N)
        throw new Error('schnorr: bad priv');
...
function nip44PadLen(unpadded) {
    if (unpadded < 1 || unpadded > 65535)
        throw new Error('nip44: bad length');
    if (unpadded <= 32)
        return 32;
...
function nip44Pad(plaintext) {
    ...
    if (pt.length < 1 || pt.length > 65535)
        throw new Error('nip44: plaintext length out of range');
...
function nip44Unpad(padded) {
    if (padded.length < 2)
        throw new Error('nip44: truncated');
    const len = padded.readUInt16BE(0);
    if (len < 1 || 2 + len > padded.length)
        throw new Error('nip44: bad plaintext len');
    if (padded.length !== 2 + nip44PadLen(len))
        throw new Error('nip44: pad mismatch');
...
function nip44Decrypt(privHex, pubXOnlyHex, payloadB64) {
    ...
    if (payload.length < 1 + 32 + 32 + 32)
        throw new Error('nip44: short payload');
    if (payload[0] !== 0x02)
        throw new Error('nip44: unsupported version');
...
function signNostrEvent(evt, privHex) {
    const out = Object.assign({ pubkey: '', created_at: 0, kind: 0, tags: [], content: '' }, evt);
...
function nip59Unwrap(recipPrivHex, wrap) {
    if (wrap.kind !== 1059)
        throw new Error('nip59: not a gift wrap');
    ...
    if (seal.kind !== 13)
        throw new Error('nip59: inner is not a seal');
    ...
    if (rumor.pubkey !== seal.pubkey)
        throw new Error('nip59: sender mismatch');
 ...
 */
function stealthSharedX(scanPrivHex, inputs) {
    const points = [];
    ...
    for (const inp of inputs) {
        ...
        if (!smallest || outpoint.compare(smallest) < 0)
            smallest = outpoint;
    }
    if (!smallest)
        return null;
    let aggX = null, aggY = 0n;
 ...
 */
function precomputeLabeledSpends(scanPrivHex, spendPubHex, labels) {
    const ms = Array.from(new Set([0, ...labels.filter(m => m && m > 0)]));
    const out = [];
    for (const m of ms) {
 ...
 */
function stealthDeriveAggregated(scanPrivHex, spendPubHex, inputs, labels = [], scriptSet = null, maxK = GAP + 1) {
 ...
 */
function stealthSpendingKey(spendPrivHex, cHex, tweakMHex = '00') {
...
function parsePaycode(paycode) {
    ...
    if (raw.length !== 132)
        throw new Error('Invalid paycode length: ' + raw.length);
    return {
...
function extractInputPubkeys(rawTxHex) {
    ...
    try {
        ...
        for (let i = 0; i < inCount; i++) {
            ...
            if (prevTxidLE.equals(Buffer.alloc(32, 0)))
                continue;
            ...
            const voutBuf = Buffer.alloc(4);
            voutBuf.writeUInt32LE(prevVout);
...
function detectPayment(params) {
    ...
    try {
        ...
        for (let i = 0; i < inCount; i++) {
            readBytes(32);
            readU32LE(); // prevTxid + vout
            const sl = readVarInt();
            readBytes(sl); // scriptSig
            readU32LE(); // sequence
        }
        const outCount = readVarInt();
        for (let i = 0; i < outCount; i++) {
            const value = buf.readBigUInt64LE(p);
            p += 8;
            const scriptLen = readVarInt();
    ...
    try {
        ...
        for (const d of derived) {
            ...
            if (false)
                break; // GAP-limit handled inside stealthDeriveAggregated
        }
    }
    catch { /* math error */ }
    return results;
 ...
 */
function deriveBip352Keys(masterPriv, masterChain, account = 0) {
    // m/352' (hardened)
            ...
            case 'derive_address': {
                ...
                if (params.senderPrivs && params.outpoints) {
                    ...
                    for (const op of params.outpoints) {
                        ...
                        const voutBuf = Buffer.alloc(4);
                        voutBuf.writeUInt32LE(op.vout || 0);
                        const outpoint = Buffer.concat([txidLE, voutBuf]);
                        if (!smallest || outpoint.compare(smallest) < 0)
                            smallest = outpoint;
                    }
                    ...
                    if (a_sum === 0n)
                        throw new Error('aggregate scalar is zero');
